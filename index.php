<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Home</title>
    <link rel="stylesheet" href="view/style.css">
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">

<link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css" rel="stylesheet">
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">


  
</head>




<body class="bg-white text-dark">

 <!--head 1-->
<header class="brand-header">
  <div class="brand-section">
    <a href="#" ></a>
    <!-- <a href="#">Tops</a>
    <a href="#">Trouser</a>
    <a href="#">Watches</a>
    <a href="#">Belt</a>
    <a href="#">Shoes</a> -->

    <a href="#"> ThreadRevolution</a>
    <a href="#">UrbanCouture </a>
    <a href="#">Plush Peach</a>
    <a href="#">Denim</a>
    <a href="#">MetroAesthetic</a>
  </div>
<div class="user-section">

 <form class="form-inline">
   
        <input class="form-control mr-sm-2" id="search-class" type="search" placeholder="Search...." aria-label="Search">
          <button type="submit"id="button-css"><i class="fas fa-search"id="magnify"></i></button>

        
  </form>

       <a href="#" class="text-dark icons-nav"><img src="view/pic/heart.png" alt="Profile" class="icon-img"id="icon-img2"></a>
     <a href="#" class="text-dark icons-nav"><img src="view/pic/placeholder.png" alt="Profile" class="icon-img"id="icon-img2"></a>
       <a href="#" class="text-dark icons-nav"><img src="view/pic/shopping-bag.png" alt="Profile" class="icon-img"id="icon-img2"></a>
        <a href="#" class="text-dark icons-nav"><img src="view/pic/user.png" alt="Profile" class="icon-img"id="icon-img2"></a>

</div>
</header>

  <!--head 1-->
   <!--head 2-->
    <header class="d-flex justify-content-between align-items-center p-4 border-bottom">
    <h1 class="h4 fw-light fst-italic"id="font-gap">SnowBee</h1>
    
        
<!-- Secondary Navigation -->
<div class="secondary-nav bg-white border-bottom">
  <div class="container">
    <ul class="nav justify-content-center py-2 position-relative">
      <li class="nav-item">
        <a class="nav-link active" href="#">New Arrivals</a>
      </li>
      
      <li class="nav-item dropdown-parent">
        <a class="nav-link" href="#">Woman</a>
        <div class="dropdown-menu-custom">
          <ul>
            <li><a href="#">Dresses</a></li>
            <li><a href="#">Tops</a></li>
            <li><a href="#">Bottoms</a></li>
            <li><a href="#">Shoes</a></li>
          </ul>
        </div>
      </li>

      <li class="nav-item dropdown-parent">
        <a class="nav-link" href="#">Man</a>
        <div class="dropdown-menu-custom">
          <ul>
            <li><a href="#">Shirts</a></li>
            <li><a href="#">Pants</a></li>
            <li><a href="#">Jackets</a></li>
            <li><a href="#">Shoes</a></li>
          </ul>
        </div>
      </li>

      <li class="nav-item">
        <a class="nav-link" href="#">Teen</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="#">Child</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="#">Baby</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="#">Home</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="#">Gifts</a>
      </li>
      <li class="nav-item">
        <a class="nav-link text-danger" href="#">Sale</a>
      </li>
    </ul>
  </div>
</div>

    </nav>
    <div class="login-bar">
 <a href="#" class="text-decoration-none text" id="login-bar-1">Sign In or Create an account</a>
    </div>

 <!--head 2-->
     
  </header>
  <nav class="navbar bg-dark py-2">
  <div class="container justify-content-center">
    <span class="navbar-text text-white fw-bold fs-5">
      40% OFF ON NEW ARRIVALS &amp; KIDS COLLECTION
    </span>
  </div>
</nav>

      
  </header>
  <!-- Hero Section -->
  <section class="hero-bg d-flex align-items-center justify-content-center text-white">
    <div class="text-center  bg-opacity-50 p-5 rounded"id="hero-head">
      <h2 class="display-4 mb-3">Step into the season’s boldest looks</h2>
     
      <a href="#" id="hero-btn"class="btn btn-light text-dark">Shop Now</a>
    </div>
  </section>



  <!-- New Arrivals -->

<section class="container py-5">
  <h3 class="mb-4">New Arrivals</h3>
<div id="carouselExample" class="carousel slide" data-bs-ride="carousel" data-bs-interval="2000" data-bs-wrap="true">

    <div class="carousel-inner">

      <!-- First Slide -->
      <div class="carousel-item active">
        <div class="row">
          <div class="col-6 col-md-3">
            <img src="https://images.unsplash.com/photo-1556905055-8f358a7a47b2" class="img-fluid rounded mb-2" alt="Product 1">
            <p>Floral Midi Dress</p>
            <p class="text-muted small">$89.99</p>
          </div>
          <div class="col-6 col-md-3">
            <img src="view/pic/f2.jpg" class="img-fluid rounded mb-2" alt="Product 2">
            <p>Linen Blazer</p>
            <p class="text-muted small">$129.99</p>
          </div>
          <div class="col-6 col-md-3 d-none d-md-block">
            <img src="view/pic/f3.jpg" class="img-fluid rounded mb-2" alt="Product 3">
            <p>Wide-Leg Pants</p>
            <p class="text-muted small">$74.99</p>
          </div>
          <div class="col-6 col-md-3 d-none d-md-block">
            <img src="view/pic/s1.jpg" class="img-fluid rounded mb-2" alt="Product 4">
            <p>Summer Top</p>
            <p class="text-muted small">$49.99</p>
          </div>
        </div>
      </div>

      <!-- Second Slide -->
      <div class="carousel-item">
        <div class="row">
          <div class="col-6 col-md-3">
            <img src="view/pic/f100.jpg" class="img-fluid rounded mb-2" alt="Product 5">
            <p>Denims Jacket</p>
            <p class="text-muted small">$99.99</p>
          </div>
          <div class="col-6 col-md-3">
            <img src="https://images.unsplash.com/photo-1512436991641-6745cdb1723f" class="img-fluid rounded mb-2" alt="Product 6">
            <p>Casual Shirt</p>
            <p class="text-muted small">$59.99</p>
          </div>
          <div class="col-6 col-md-3 d-none d-md-block">
            <img src="view/pic/s11.jpg" class="img-fluid rounded mb-2" alt="Product 7">
            <p>Sport Sneakers</p>
            <p class="text-muted small">$79.99</p>
          </div>
          <div class="col-6 col-md-3 d-none d-md-block">
            <img src="view/pic/s20.jpg" class="img-fluid rounded mb-2" alt="Product 8">
            <p>Classic Watch</p>
            <p class="text-muted small">$149.99</p>
          </div>
        </div>
      </div>

    </div>

    <!-- Controls -->
    <button class="carousel-control-prev" type="button" data-bs-target="#carouselExample" data-bs-slide="prev">
      <span class="carousel-control-prev-icon" aria-hidden="true"></span>
    </button>
    <button class="carousel-control-next" type="button" data-bs-target="#carouselExample" data-bs-slide="next">
      <span class="carousel-control-next-icon" aria-hidden="true"></span>
    </button>
  </div>
</section>

<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>




<section class="container my-5">
  <h2 class="mb-4 text-center">Women's Collection</h2>
  <div class="row g-4">
    <div class="col-sm-6 col-md-4">
      <div class="long-fashion-card">
        <img src="view/pic/f3.jpg" alt="Elegant Dress">
        <div class="card-details">
          <div class="card-title">Elegant Dress</div>
          <div class="card-price">$49.99</div>
        </div>
      </div>
    </div>

    <div class="col-sm-6 col-md-4">
      <div class="long-fashion-card">
        <img src="view/pic/f2.jpg" alt="Casual Wear">
        <div class="card-details">
          <div class="card-title">Casual Wear</div>
          <div class="card-price">$39.99</div>
        </div>
      </div>
    </div>

    <div class="col-sm-6 col-md-4">
      <div class="long-fashion-card">
        <img src="view/pic/f1.jpg" alt="Summer Outfit">
        <div class="card-details">
          <div class="card-title">Summer Outfit</div>
          <div class="card-price">$59.99</div>
        </div>
      </div>
    </div>


  
  </div>
</section>
<!-- <div class="container-fluid">
  <div class="row">
   
    <div class="col-md-6 shop-section" style="background-image: url('view/pic/a1.jpg');">
      <a href="#" class="overlay-button">Shop</a>
    </div>


    
    <div class="col-md-6 shop-section" style="background-image: url('view/pic/a2.jpg');">
      <a href="#" class="overlay-button">Shop</a>
    </div>
  </div>
</div> -->

<div class="container-fluid">
  <div class="row">
 
    <div class="col-md-6 shop-section position-relative p-0">
      <div class="image-container">
        <div class="bg-image" style="background-image: url('view/pic/a45.jpg');"></div>
             <!-- <div class="bg-image" style="background-image: url('view/pic/f1000.jpg');"></div> -->
        <div class="info-overlay">
          <h4>Girl's Collection</h4>
          <p>Explore the latest collection for Girls</p>
        </div>
      </div>
    </div>

    
    <div class="col-md-6 shop-section position-relative p-0">
      <div class="image-container">
        <div class="bg-image" style="background-image: url('view/pic/a2.jpg');"></div>
        <div class="info-overlay">
          <h4>Boy's Collection</h4>
          <p>New styles curated just for you</p>
        </div>
      </div>
    </div>
  </div>
</div>


<section class="fashion-banner">
  <div class="banner-image">
    <img src="view/pic/men1.jpg" alt="Women's Jacket Collection">
  </div>
  <div class="banner-content">
    <h2>Men's Collection</h2>
    <p>Discover our premium selection of cozy teddy coats, classic wool styles, and chic silhouettes — tailored to elevate your wardrobe this season.</p>
    <a href="#" class="shop-button">Check Now</a>
  </div>
</section>

    <!--------------------------------------------  NEWS -------------------------------------------------- Letter ------->
  <section class="bg-light text-center py-5">
    <h4 class="mb-3">Join Our Style Circle</h4>
    <p class="mb-4">Get exclusive updates, early access & 10% off your first order</p>
    <form class="row justify-content-center g-2">
      <div class="col-md-4">
        <input type="email" class="form-control" id="e-mail-form" placeholder="Your email address">
      </div>
      <div class="col-auto">
        <button type="submit" class="btn btn-dark"id="sub-id">Subscribe</button>
      </div>
    </form>
  </section>
    <!--------------------------------------------  KIDS -------------------------------------------------- SECTION ------->

<section class="banner-bg d-flex align-items-center justify-content-center text-white">
    <div class="text-center  bg-opacity-50 p-5 rounded"id="hero-head2">
      <h2 class="display-4 mb-3" id="kids-her">See more Kid's Collection</h2>
     
      <a href="#" id="hero-btn"class="btn btn-light text-dark">Shop Now</a>
    </div>
  </section>
   <!--------------------------------------------  KIDS -------------------------------------------------- SECTION ------->
  <!--------------------------------------------  NEWS -------------------------------------------------- Letter ------->

  <!--------------------------------------------  NEWS -------------------------------------------------- SECTION ------->
  <section class="news-feed py-5 bg-light">
  <div class="container">
    <h2 class="text-center mb-5 text-dark text-uppercase">News Feed</h2>
    <div class="row g-4">

    
      <div class="col-md-4">
        <div class="card h-100 border-0 shadow-sm">
          <img src="view/pic/n1.jpg" class="card-img-top" alt="News Image 1">
          <div class="card-body">
            <h5 class="card-title text-dark">Summer 2025's Fashion Trends Champion Low-Effort, High-Reward Style</h5>
            <p class="card-text text-secondary">Explore how eco-friendly fabrics and ethical production are changing the fashion industry for the better.</p>
            <a href="#" class="text-decoration-none text-dark fw-semibold">Read More </a>
          </div>
        </div>
      </div>

    
      <div class="col-md-4">
        <div class="card h-100 border-0 shadow-sm">
          <img src="view/pic/n2.jpg" class="card-img-top" alt="News Image 2">
          <div class="card-body">
            <h5 class="card-title text-dark">Top Trends This Season</h5>
            <p class="card-text text-secondary">Discover the hottest color palettes, patterns, and silhouettes dominating runways and streetwear alike.</p>
            <a href="#" class="text-decoration-none text-dark fw-semibold">Read More </a>
          </div>
        </div>
      </div>

      
      <div class="col-md-4">
        <div class="card h-100 border-0 shadow-sm">
          <img src="view/pic/n3.jpg" class="card-img-top" alt="News Image 3">
          <div class="card-body">
            <h5 class="card-title text-dark">Meet the Faces of Fall 2025</h5>
            <p class="card-text text-secondary">Get to know the new wave of fashion icons and influencers making waves in the global scene.</p>
            <a href="#" class="text-decoration-none text-dark fw-semibold">Read More </a>
          </div>
        </div>
      </div>

    </div>
  </div>
</section>






<div class="container-fluid">
  <div class="row">
 
    <div class="col-md-6 shop-section position-relative p-0">
      <div class="image-container">
        <div class="bg-image" style="background-image: url('view/pic/f212.jpg');"></div>
             <!-- <div class="bg-image" style="background-image: url('view/pic/f1000.jpg');"></div> -->
        <div class="info-overlay">
          <h4>Men's</h4>
          <p>Explore the latest collection for men</p>
        </div>
      </div>
    </div>

    
    <div class="col-md-6 shop-section position-relative p-0">
      <div class="image-container">
        <div class="bg-image" style="background-image: url('view/pic/f7.jpg');"></div>
        <div class="info-overlay">
          <h4>Women's</h4>
          <p>New styles curated just for you</p>
        </div>
      </div>
    </div>
  </div>
</div>






  <!--------------------------------------------  NEWS -------------------------------------------------- SECTION ------->
    <!--------------------------------------------  NEWS -------------------------------------------------- SECTION ------->
  <!-- Footer -->
<footer class="bg-light text-secondary pt-5 pb-4 border-top">
  <div class="container-fluid px-5">
    <div class="row">

      <!-- About -->
      <div class="col-12 col-md-4 mb-4">
        <h5 class="text-dark text-uppercase mb-3">About Us</h5>
        <p style="line-height: 1.7;">
          Explore the essence of modern fashion. From essential basics to exclusive collections, our brand brings you style that evolves with you.
        </p>
        <a href="#" class="text-decoration-none text-secondary">Find a Store Near You <i class="fas fa-map-marker-alt ms-1"></i></a>
      </div>

      <!-- Quick Links -->
      <div class="col-6 col-md-2 mb-4">
        <h6 class="text-dark text-uppercase mb-3">Quick Links</h6>
        <ul class="list-unstyled">
          <li><a href="#" class="text-secondary text-decoration-none">Home</a></li>
          <li><a href="#" class="text-secondary text-decoration-none">Shop</a></li>
          <li><a href="#" class="text-secondary text-decoration-none">New Arrivals</a></li>
          <li><a href="#" class="text-secondary text-decoration-none">Sale</a></li>
           <li><a href="#" class="text-secondary text-decoration-none">Shirts</a></li>
          <li><a href="#" class="text-secondary text-decoration-none">Pants</a></li>
          <li><a href="#" class="text-secondary text-decoration-none">Watches</a></li>
          <li><a href="#" class="text-secondary text-decoration-none">Men's</a></li>
                    <li><a href="#" class="text-secondary text-decoration-none">Women's</a></li>
                              <li><a href="#" class="text-secondary text-decoration-none">Teen's</a></li>
                                                            <li><a href="#" class="text-secondary text-decoration-none">Kid</a></li>
                                                                                          <li><a href="#" class="text-secondary text-decoration-none">Babies</a></li>
        </ul>
      </div>

      <!-- Customer Care -->
      <div class="col-6 col-md-2 mb-4">
        <h6 class="text-dark text-uppercase mb-3">Customer Care</h6>
        <ul class="list-unstyled">
          <li><a href="#" class="text-secondary text-decoration-none">Contact Us</a></li>
          <li><a href="#" class="text-secondary text-decoration-none">FAQs</a></li>
          <li><a href="#" class="text-secondary text-decoration-none">Shipping Info</a></li>
          <li><a href="#" class="text-secondary text-decoration-none">Return Policy</a></li>
                <li><a href="#" class="text-secondary text-decoration-none">Guidelines</a></li>
                          <li><a href="#" class="text-secondary text-decoration-none">Contact Us</a></li>
          <li><a href="#" class="text-secondary text-decoration-none">FAQs</a></li>
          <li><a href="#" class="text-secondary text-decoration-none">Shipping Info</a></li>
          <li><a href="#" class="text-secondary text-decoration-none">Return Policy</a></li>
                <li><a href="#" class="text-secondary text-decoration-none">Guidelines</a></li>
        </ul>
      </div>

   
      <div class="col-12 col-md-4 mb-4">
        <h6 class="text-dark text-uppercase mb-3">Subscribe to Our Newsletter</h6>
        <form class="d-flex flex-column flex-sm-row mb-3">
          <input type="email" class="form-control me-sm-2 mb-2 mb-sm-0" id="newsletter-2"placeholder="Enter your email">
          <button class="btn btn-dark" type="submit">Subscribe</button>
        </form>

     
        <div class="mb-3">
          <span class="me-2">Follow us:</span>
          <a href="#" class="text-secondary me-2"><i class="fab fa-facebook-f"></i></a>
          <a href="#" class="text-secondary me-2"><i class="fab fa-instagram"></i></a>
          <a href="#" class="text-secondary me-2"><i class="fab fa-twitter"></i></a>
          <a href="#" class="text-secondary"><i class="fab fa-youtube"></i></a>
        </div>

       
        <div>
          <img src="https://img.icons8.com/color/48/000000/visa.png" alt="Visa" height="30">
          <img src="https://img.icons8.com/color/48/000000/mastercard.png" alt="Mastercard" height="30">
          <img src="https://img.icons8.com/color/48/000000/paypal.png" alt="PayPal" height="30">
          <img src="https://img.icons8.com/color/48/000000/amex.png" alt="Amex" height="30">
        </div>
      </div>

    </div>

    <hr class="border-dark">

    <div class="text-center text-secondary">
      <small>&copy; 2025 SnowBee || All rights reserved.</small>
    </div>
  </div>
</footer>



  <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>


</body>
</html>